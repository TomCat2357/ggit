...
2. **Commit…** → `.vcs` タブが生成/更新され、JSON にコミットが入る。
3. 本文を編集 → 再度 **Commit…** → **Log** に 2 コミット、**Diff…** で差分が着色表示。
4. **Branch…** → 新タブが複製生成され、HEAD が分岐元を指す。
5. 2 タブで別々に編集・コミット後、**Merge…** → クリーン統合、または競合マーカー書き戻しを目視確認。

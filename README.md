...
> これにより、**最初の Commit が認可中断で `.vcs` ごと失われる事象**を避けられます。
...
確認します（hash・スナップショット往復・diff3・LCA・v3 移行・グラフ描画・タブ本文 end-index を検証）。

### B. 手動 E2E チェックリスト

1. `npm run push` 後、ドキュメントを開き直し、メニュー **ggit** が出る。
2. **Setup / 権限付与** → 権限承認ダイアログを承認し、ドキュメント名・タブ数の初期化完了表示を確認。
3. **Commit…** → `.vcs` タブが生成/更新され、JSON に version 3 ストア（`working` 設定）が入る（初回承認直後は 1 度キャンセルされることがあるため、その場合は再実行）。
4. 本文を編集 → 再度 **Commit…** → **Log（グラフ）** に 2 コミット、`@` が前進、**Diff…** で差分が着色表示。
5. **Bookmark…** で現在地に `main` を付与 → グラフに `<main>` chip。
6. あるコミットへ **Goto…** → 内容が差し替わり、未コミット分が `[stash]` ノードとして残る。別の編集を Commit → **Log（グラフ）** に分岐（`|/`）が表示。
7. スタッシュの **戻す(pop)** → 内容が戻り、その `[stash]` が消える。
8. **Merge…**（ブックマーク or コミット）→ クリーン統合（マージコミット）、または競合マーカー書き戻しを目視確認。
